...
== CHALLENGE: Push a image to the OpenShift registry

On real scenarios environments it will be disconnected or cannot use a foreign registry, to represent this scenario, this lab has a challenge.

For next sessions work as expected the container image on bastion should be present in OpenShift.

So you need to figure out how to import it to OCP. 

TIP: You can change your Ansible inventory, or expose the OpenShift registry or even deal with rbacs.

The important is not to have the *ImagePullBackOff* error.

TIP: If you give up read this [solution]https://access.redhat.com/solutions/6959306 
