...
mtv_migrate_migration_request:
  ...
  vms:
    - path: "/SDDC-Datacenter/vm/Workloads/sandbox-<GUID>/rhel9-vma-superlab-1"
    - path: "/SDDC-Datacenter/vm/Workloads/sandbox-<GUID>/rhel9-vma-superlab-2"

----

Once again, be sure to update `<GUID>` in the `vms` property with your student ID.
...
Notice at the end of the playbook in the task `Update MAC Address`, the `rhel9-vma-superlab-1` Virtual Machine was Updated:

[source,shell]
----
TASK [Update MAC Address] ****************************
changed: [localhost] => (item=rhel9-vma-superlab-1)
----
...
[source,shell,role=execute]
----
oc get vm -n vmexamples-automation rhel9-vma-superlab-1 -o jsonpath='{ .spec.template.spec.domain.devices.interfaces[*].macAddress }'
----
